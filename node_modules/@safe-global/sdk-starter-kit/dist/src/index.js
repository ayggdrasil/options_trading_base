        ...
        const initConfig = {
            ...
            predictedSafe: {
                ...
                safeDeploymentConfig: {
                    ...
                    deploymentType: constants_1.DEFAULT_DEPLOYMENT_TYPE
                }
            }
        };
        try {
            protocolKit = await protocol_kit_1.default.init(initConfig);
        }
        catch (error) {
            const isDeploymentTypeUnresolvedError = error instanceof Error &&
                error.message &&
                error.message.startsWith('Invalid') &&
                error.message.includes('contract address');
            if (isDeploymentTypeUnresolvedError &&
...
async function getApiKitInstance(protocolKit, config) {
    ...
    return new api_kit_1.default({ chainId, txServiceUrl: config.txServiceUrl });
}
