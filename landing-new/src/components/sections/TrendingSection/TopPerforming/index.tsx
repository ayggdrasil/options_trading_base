...
import { TopPerformingOption } from "../../../../contexts/data/TrendingContext";
...
const TopPerforming: React.FC = () => {
  ...
  const equityOptions: TopPerformingOption[] = [];
  const cryptoOptions: TopPerformingOption[] = [];

  leaderBoardData.topPerformingOptions.forEach((item) => {
  ...
  return (
    <div className="flex flex-col gap-[12px] lg:flex-row lg:gap-[24px]">
      ...
      <OptionsTable title="Equity Options" items={equityOptions} mode="change" />

      {/* Crypto Options Table */}
      <OptionsTable title="Crypto Options" items={cryptoOptions} mode="change" />
    </div>
  );
};
