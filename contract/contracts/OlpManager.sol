...
contract OlpManager is IOlpManager, OwnableUpgradeable, ReentrancyGuardUpgradeable, AuthorityUtil {
    ...
    function addLiquidity(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minOlp) external override nonReentrant returns (uint256) {
        require(!inPrivateMode, "OlpManager: action not enabled");
    ...
    function isWhitelistedToken(address _token) public view override returns (bool) {
        return vault.isWhitelistedToken(_token);
    }

    function isEnabledToken(address _token) public view override returns (bool) {
        return vault.isEnabledToken(_token);
    }

    function isCooldownPassed(address _account) public view override returns (bool) {
        return lastAddedAt[_account] + cooldownDuration <= block.timestamp;
    }
    ...
    function _getOlpAssetAmounts(address _token) internal view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        require(isWhitelistedToken(_token), "OlpManager: invalid token");
    ...
    function _removeLiquidity(address _account, address _tokenOut, uint256 _olpAmount, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        require(isCooldownPassed(_account), "OlpManager: cooldown duration not yet passed");
