...
import "../../contracts/libraries/StringsUpgradeable.sol";
...
contract TestUtils is Test {
    ...
    function fromUnit(uint256 amount, uint256 unit, uint256 digit) internal pure returns (string memory result){
        ...
        string memory amountToString = StringsUpgradeable.toString(amount);
    ...
    function decimals(address token) internal view returns (uint256) {
        uint256 decimal = token == address(0) ? 18 : IERC20(token).decimals();
        return decimal;
    }

    function symbol(address token) internal view returns (string memory) {
        string memory _symbol = token == address(0) ? "KLAY" : IERC20(token).symbol();
        return _symbol;
    }
    ...
    function _upgradeProxy(
        ...
        ProxyAdmin(_proxyAdmin).upgrade(TransparentUpgradeableProxy(payable(proxy)), _implementation);
    }
